...
class MatDatasetOffline(torch.utils.data.Dataset):
    def __init__(self, args, transform=None, normalize=None):
        ...
        imgbig_paths = get_files(self.args.imgbigDir)

        self.cnt = len(imgbig_paths)

        for imgbig_path in imgbig_paths:
            # print(imgbig_path)
            alpha_path = imgbig_path.replace(self.args.imgbigDir, self.args.alphaDir)
            img_path = imgbig_path.replace(self.args.imgbigDir, self.args.imgDir)
            bg_path = imgbig_path.replace(self.args.imgbigDir, self.args.bgDir)
            fg_path = imgbig_path.replace(self.args.imgbigDir, self.args.fgDir)
            # assert(os.path.exists(alpha_path))
            # assert(os.path.exists(fg_path))
            # assert(os.path.exists(bg_path))
            # assert(os.path.exists(img_path))
            self.samples.append((alpha_path, fg_path, bg_path, img_path,imgbig_path))
        self.logger.info("MatDatasetOffline Samples: {}".format(self.cnt))
        assert(self.cnt > 0)

    def __getitem__(self,index):
        alpha_path, fg_path, bg_path, img_path, imgbig_path= self.samples[index]

        img_info = [fg_path, alpha_path, bg_path, img_path, imgbig_path]
        ...
        img_big = cv2.imread(imgbig_path)[:, :, :3]
        img_big_w = img_big.shape[1]
        img_big_h = img_big.shape[0]
        img_w = int(img_big_w/4)

        #[y0:y1,x0:x1]
        img = img_big[0:img_big_h, img_w * 0:img_w * 0 + img_w]
        fg = img_big[0:img_big_h, img_w * 1:img_w * 1 + img_w]
        bg = img_big[0:img_big_h, img_w * 2:img_w * 2 + img_w]
        alpha = img_big[0:img_big_h, img_w * 3:img_w * 3 + img_w]
